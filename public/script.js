...
gameStates[2] = async function(){
    ...
    if(score > highScore){
        highScore = score;
        //add score to DB
        scoreContainer.value = highScore;
        ctx.save();
        ctx.font = "30px Arial";
        ctx.fillStyle = "white";
        ctx.textAlign = "center"
        ctx.fillText("Game Over, Your score was: " + score.toString(), c.width/2, c.height/4 - 60);
        ctx.fillText("Your New High Score is: " + highScore.toString() , c.width/2, c.height/4 - 30);
        ctx.fillText("New Record!!", c.width/2, c.height/4 );
        ctx.font = "15px Arial";
        ...
        try{
            ...
            //scoreArray.forEach((highScore)=>{
            //    ctx.fillText(highScore.score, c.width/2, y);
            //    y += 20;
            //});
        }
        catch(error)
        ...
        ctx.textAlign = "center"
        ctx.fillText("Game Over, Your score was: " + score.toString(), c.width/2, c.height/4 - 60);
        ctx.fillText("Your high Score is: " + highScore.toString(), c.width/2, c.height/4 - 30);
        ctx.font = "15px Arial";
        ctx.fillText("Press Enter to Start", c.width/2, c.height/4 + 20);
        try{
